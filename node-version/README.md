...
### Terminal shortcut (Linux)

`pacman.sh` wraps all of the above — it installs dependencies on first run,
rebuilds only when sources changed, then serves the build and opens a browser.

```bash
chmod +x pacman.sh
ln -s "$PWD/pacman.sh" ~/.local/bin/pacman-game   # needs ~/.local/bin on PATH
```

Then from any directory:

```bash
pacman-game          # build if needed + serve on http://localhost:4173
pacman-game dev      # dev server on http://localhost:5173
pacman-game build    # rebuild dist/ and exit
pacman-game reset    # wipe the leaderboard
```

Prefer an alias instead of a symlink? Add this to `~/.bashrc` or `~/.zshrc`:

```bash
alias pacman-game='~/Documents/GitHub/pacman-remake/pacman.sh'
```
